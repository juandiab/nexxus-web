<template>
  <div ref="containerRef" :class="['strands-container', className]" :style="style" />
</template>
...
function update(t) {
  ...
  const time = prefersReducedMotion ? 0 : t * 0.001
  syncUniforms(time)

  if (props.glass && renderTarget && glassProgram && glassMesh) {
...
watch(
  ...
  () => syncUniforms(program?.uniforms.uTime.value ?? 0),
  { deep: true },
)
</script>
