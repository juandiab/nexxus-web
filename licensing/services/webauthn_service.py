...
async def list_user_passkeys(db: AsyncIOMotorDatabase, user_id: ObjectId) -> list[dict[str, Any]]:
    return await db.passkeys.find({"userId": user_id}).sort("createdAt", -1).to_list(length=None)
...
async def get_passkey_by_credential_id(db: AsyncIOMotorDatabase, credential_id: str) -> dict | None:
...
async def begin_registration(db: AsyncIOMotorDatabase, user: dict[str, Any]) -> dict[str, Any]:
...
async def finish_authentication(
    ...
    await db.passkeys.update_one(
        {"_id": stored["_id"]},
        {"$set": {"signCount": verification.new_sign_count, "lastUsedAt": utc_now()}},
    )
